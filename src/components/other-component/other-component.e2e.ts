...
describe(componentName, () => {
  ...
  it('renders changes to the name data', async () => {
    ...
    const element = await page.find(componentName);
    expect(element.innerText).toEqual(`Hello, World! I'm  `);

    component.setProperty('first', 'James');
    await page.waitForChanges();
    expect(element.innerText).toEqual(`Hello, World! I'm James`);

    component.setProperty('last', 'Quincy');
    await page.waitForChanges();
    expect(element.innerText).toEqual(`Hello, World! I'm James Quincy`);

    component.setProperty('middle', 'Earl');
    await page.waitForChanges();
    expect(element.innerText).toEqual(`Hello, World! I'm James Earl Quincy`);
  });
});
